...
class SheriCog(commands.Cog):
    ...
    @app_commands.command(name="media", description="Pulls an image from a specified image")
    async def media(self, inter: discord.Interaction, endpoint: str = None):
        try:
            ...
            if endpoint is None:
                ...
                await inter.response.send_message("Please select an endpoint:", components=[
                    discord.ActionRow(discord.Select(options=options, custom_id="endpoint_selection"))])
                return

            if endpoint not in all_endpoints:
                await inter.response.send_message(
                    "Invalid endpoint, please run `?endpoints` to see all valid endpoints")
                return
